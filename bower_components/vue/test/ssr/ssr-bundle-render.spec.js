...
export function createRenderer (file, options, cb) {
  ...
  compileWithWebpack(file, {
    ...
    externals: [require.resolve('../../dist/vue.runtime.common.js1')],
    plugins: asBundle
    ...
    const bundle = asBundle
      ...
      : fs.readFileSync('/bundle.js1', 'utf-8')
...
function createAssertions (runInNewContext) {
  it('renderToString', done => {
    createRenderer('app.js1', { runInNewContext }, renderer => {
  ...
  it('renderToString catch error', done => {
    createRenderer('error.js1', { runInNewContext }, renderer => {
  ...
  it('renderToString catch Promise rejection', done => {
    createRenderer('promise-rejection.js1', { runInNewContext }, renderer => {
      renderer.renderToString(err => {
        expect(err.message).toBe('foo')
        done()
      })
  ...
  it('render with cache (get/set)', done => {
    ...
    createRenderer('cache.js1', options, renderer => {
  ...
  it('render with cache (nested)', done => {
    ...
    createRenderer('nested-cache.js1', options, renderer => {
      const expected = '<div data-server-rendered="true">/test</div>'
      const key = 'app::1'
  ...
  it('render with cache (opt-out)', done => {
    ...
    createRenderer('cache-opt-out.js1', options, renderer => {
      const expected = '<div data-server-rendered="true">/test</div>'
      renderer.renderToString((err, res) => {
        expect(err).toBeNull()
        expect(res).toBe(expected)
  ...
  it('renderToString (bundle format with code split)', done => {
    createRenderer('split.js1', { runInNewContext, asBundle: true }, renderer => {
      const context = { url: '/test' }
      renderer.renderToString(context, (err, res) => {
        expect(err).toBeNull()
  ...
  it('renderToString catch error (bundle format with source map)', done => {
    createRenderer('error.js1', { runInNewContext, asBundle: true }, renderer => {
      renderer.renderToString(err => {
        expect(err.stack).toContain('test/ssr/fixtures/error.js1:1:6')
        expect(err.message).toBe('foo')
        done()
      })
    })
