...
class MY_Model extends CI_Model
{
    ...
    public function delete($id)
    {
        ...
        if ($this->soft_delete) {
            $this->_database->update($this->_table, array($this->soft_delete_key => true));
        } else {
    ...
    /**
     * Delete many rows from the database table based on an arbitrary WHERE call.
     */
    ...
    public function genericFilter($model, $limit, $offset, $filter = array())
    {

        $query = (array)json_decode($filter, true);
        $this->db->select($query["select"])->from($model);

        if (!empty(array_keys($query['join']))) {
            foreach ($query["join"] as $item) {
//                var_dump($value);
                $array_keys = array_keys($item);
                $this->db->join($array_keys[0], $item[$array_keys[0]]);
            }
            if (!empty(array_keys($query["where"]))) {
    ...
    /**
     * Fetch all the records in the table. Can be used as a generic call. Use in JqueryDataTable
     */
    public function get_all_dataTable($data, $sort)
    {
        ...
        $query = $this->_database->get($this->_table, $data['length'], $data['start'])->result_array();
        ...
        if ($query) {
            ...
            for ($i = 0; $i < count($query); $i++) {
                ...
                for ($o = 0; $o < count($key); $o++) {
                    $result[$i][$o] = $query[$i][$key[$o]];
                }
            }
        } else {
            $result = [];
    ...
    public function search($tableName, $fields = array(), $search = "")
    {
        $select = "*";
    ...
    {
        $query = (array)json_decode($data["rawQuery"], true);
        $table = ($data["table"]);

        $this->db->select($query["select"])->from($table);
        if (count($query["where"])) {
            $this->db->where($query["where"]);
        }
        $total = $this->count_ext();
        return array('total' => $total, 'results' => $this->get_all_ext());


    }


    public function listTable($limit, $offset, $filter = array())
    {
        $this->db->select("*")->from($this->_table);
        if (array_key_exists("filter", $filter)) {
            $query = (array)json_decode($filter["filter"], true);
            $this->db->where($query);
        }
